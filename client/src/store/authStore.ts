...
interface AuthState {
  ...
  directLogin: (userId: number) => Promise<AuthResponse>
...
export const useAuthStore = create<AuthState>()(
  ...
  directLogin: async (userId: number) => {
    authSequence++
    set({ isLoading: true, error: null })
    try {
      const data = await authApi.directLogin(userId)
      set({
        user: data.user,
        isAuthenticated: true,
        isLoading: false,
      ...
      const error = getApiErrorMessage(err, 'Direct login failed')
      set({ isLoading: false, error })
      throw new Error(error)
    }
  },
