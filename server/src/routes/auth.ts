...
router.get('/public-users', (req: Request, res: Response) => {
  res.json({ users: listPublicUsers() });
});

router.post('/direct-login', authLimiter, (req: Request, res: Response) => {
  const { user_id } = req.body;
  if (!user_id) return res.status(400).json({ error: 'user_id is required' });
  
  const result = directLoginUser(user_id);
  if (result.auditAction) {
    writeAudit({ userId: result.auditUserId ?? null, action: result.auditAction, ip: getClientIp(req), details: { direct: true } });
  }
