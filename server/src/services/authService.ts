...
export function listPublicUsers() {
  ...
  ).all() as Pick<User, 'id' | 'username' | 'avatar'>[];
  return users.map(u => ({ ...u, avatar_url: avatarUrl(u) }));
}

export function directLoginUser(userId: number): {
  error?: string;
  status?: number;
  token?: string;
  user?: Record<string, unknown>;
  ...
  if (!user) {
    return {
      ...
      auditUserId: null, auditAction: 'user.direct_login_failed'
    };
  }
